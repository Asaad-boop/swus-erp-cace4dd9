...
function ReportCard({ children }: { children: React.ReactNode }) {
  return <div className="bg-white rounded-xl border border-[color:var(--hr-border)] shadow-sm p-5 space-y-4">{children}</div>;
}

const thCls = "bg-muted/40 text-[11px] uppercase tracking-wider text-[color:var(--hr-text-muted)] font-semibold";
...
function HeadcountReport() {
  ...
  return (
    <ReportCard>
      ...
      <div className="text-sm text-[color:var(--hr-text-muted)]">Total: <span className="font-semibold text-[color:var(--hr-text-strong)] tabular-nums">{total}</span></div>
      <Table>
        <TableHeader><TableRow className="border-[color:var(--hr-border)] hover:bg-transparent"><TableHead className={thCls}>{groupBy.replace("_"," ")}</TableHead><TableHead className={`${thCls} text-right`}>Count</TableHead><TableHead className={`${thCls} text-right`}>%</TableHead></TableRow></TableHeader>
        <TableBody>
          {(data as any[]).map((r, i) => (
            <TableRow key={i} className="border-[color:var(--hr-border)]"><TableCell>{r.label}</TableCell><TableCell className="text-right font-semibold tabular-nums">{r.count}</TableCell><TableCell className="text-right text-[color:var(--hr-text-muted)] tabular-nums">{total ? Math.round((r.count/total)*100) : 0}%</TableCell></TableRow>
          ))}
        </TableBody>
      </Table>
    </ReportCard>
  ...
  return (
    <ReportCard>
      <div className="flex justify-between items-end flex-wrap gap-2">
        <div className="flex gap-2 items-end">
          <div><Label>From</Label><Input type="date" value={from} onChange={(e) => setFrom(e.target.value)} className="h-9 rounded-lg border-[color:var(--hr-border)]" /></div>
          <div><Label>To</Label><Input type="date" value={to} onChange={(e) => setTo(e.target.value)} className="h-9 rounded-lg border-[color:var(--hr-border)]" /></div>
        </div>
      ...
      <div className="overflow-x-auto">
        <Table>
          ...
          <TableBody>
            {(data as any[]).map((r, i) => (
              <TableRow key={i} className="border-[color:var(--hr-border)]">
                <TableCell className="font-medium text-[color:var(--hr-text-strong)]">{r.name} <span className="text-xs text-[color:var(--hr-text-muted)] font-mono">({r.employee_code})</span></TableCell>
  ...
  return (
    <ReportCard>
      <div className="flex justify-between items-end">
        <div>
          <Label>Year</Label>
          <Input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} className="w-28 h-9 rounded-lg border-[color:var(--hr-border)]" />
        </div>
...
function PayrollReport() {
  ...
  return (
    <ReportCard>
      <div className="flex justify-between items-end">
        <div><Label>Year</Label><Input type="number" value={year} onChange={(e) => setYear(Number(e.target.value))} className="w-28 h-9 rounded-lg border-[color:var(--hr-border)]" /></div>
      ...
      <div className="text-sm text-[color:var(--hr-text-muted)]">Total net paid this year: <span className="font-bold text-[color:var(--hr-text-strong)] tabular-nums">৳{total.toLocaleString("en-BD")}</span></div>
      <Table>
        <TableHeader><TableRow className="border-[color:var(--hr-border)] hover:bg-transparent"><TableHead className={thCls}>Month</TableHead><TableHead className={thCls}>Employee</TableHead><TableHead className={`${thCls} text-right`}>Gross</TableHead><TableHead className={`${thCls} text-right`}>Net</TableHead><TableHead className={thCls}>Status</TableHead></TableRow></TableHeader>
        <TableBody>
          {(data as any[]).map((r, i) => (
            <TableRow key={i} className="border-[color:var(--hr-border)]">
              ...
              <TableCell className="text-xs text-[color:var(--hr-text-muted)]">{r.payment_status}</TableCell>
            </TableRow>
          ))}
        </TableBody>
      </Table>
...
function AnnivReport() {
  ...
  return (
    <div className="grid md:grid-cols-2 gap-5">
      <ReportCard>
        <div className="flex justify-between items-center">
          <h3 className="font-semibold text-[color:var(--hr-text-strong)]">Birthdays (next 30 days)</h3>
        ...
        {bdays.length === 0 ? <div className="text-sm text-[color:var(--hr-text-muted)]">No upcoming birthdays.</div> : (
          <Table>
            <TableHeader><TableRow className="border-[color:var(--hr-border)] hover:bg-transparent"><TableHead className={thCls}>Name</TableHead><TableHead className={thCls}>Date</TableHead><TableHead className={`${thCls} text-right`}>In Days</TableHead></TableRow></TableHeader>
            <TableBody>{bdays.map((b: any) => <TableRow key={b.id} className="border-[color:var(--hr-border)]"><TableCell className="font-medium">{b.name}</TableCell><TableCell className="font-mono text-xs">{b.date}</TableCell><TableCell className="text-right tabular-nums">{b.in}</TableCell></TableRow>)}</TableBody>
          </Table>
        )}
      </ReportCard>
      <ReportCard>
        <div className="flex justify-between items-center">
          <h3 className="font-semibold text-[color:var(--hr-text-strong)]">Work Anniversaries (next 30 days)</h3>
        ...
        {annivs.length === 0 ? <div className="text-sm text-[color:var(--hr-text-muted)]">No upcoming anniversaries.</div> : (
          <Table>
            <TableHeader><TableRow className="border-[color:var(--hr-border)] hover:bg-transparent"><TableHead className={thCls}>Name</TableHead><TableHead className={thCls}>Date</TableHead><TableHead className={`${thCls} text-right`}>Years</TableHead></TableRow></TableHeader>
            <TableBody>{annivs.map((a: any) => <TableRow key={a.id} className="border-[color:var(--hr-border)]"><TableCell className="font-medium">{a.name}</TableCell><TableCell className="font-mono text-xs">{a.date}</TableCell><TableCell className="text-right tabular-nums font-semibold">{a.years}</TableCell></TableRow>)}</TableBody>
          </Table>
        )}
      </ReportCard>
    </div>
