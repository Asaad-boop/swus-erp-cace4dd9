...
function SuppliersPage() {
  ...
  const { data: suppliers = [], isLoading } = useQuery({
    queryKey: ["suppliers", activeBrand?.id],
      ...
      <header className="flex flex-wrap items-end justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground">
            {activeBrand?.name} · {suppliers.length} suppliers · Total due: <span className="font-semibold text-foreground">৳{totalDue.toLocaleString()}</span>
          </p>
        </div>
