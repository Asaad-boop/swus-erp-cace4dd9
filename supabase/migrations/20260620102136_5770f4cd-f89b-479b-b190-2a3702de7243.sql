...
BEGIN
  ...
  WITH mkt AS (
    ...
    SELECT
      'manual'::text AS kind,
      COALESCE(category, 'manual')::text AS expense_type,
      amount::numeric AS amount,
      note,
  ...
  SELECT
    ...
    COALESCE(SUM(CASE WHEN expense_type IN ('meta_ads','meta_ads_manual') OR kind='manual' AND expense_type IN ('boosting','meta','facebook_ads') THEN amount ELSE 0 END),0),
