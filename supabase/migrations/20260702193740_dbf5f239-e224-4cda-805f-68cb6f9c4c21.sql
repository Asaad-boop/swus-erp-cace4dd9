...
-- Recalculate all reserved_stock with new rule (confirmed only)
UPDATE public.products p SET reserved_stock = COALESCE(sub.qty, 0)
FROM (
  SELECT oi.product_id, SUM(oi.quantity)::int AS qty
  FROM order_items oi
  JOIN orders o ON o.id = oi.order_id
  WHERE oi.variant_id IS NULL AND o.status = 'confirmed'
...
FROM (
  ...
  FROM order_items oi
  JOIN orders o ON o.id = oi.order_id
  WHERE oi.variant_id IS NOT NULL AND o.status = 'confirmed'
