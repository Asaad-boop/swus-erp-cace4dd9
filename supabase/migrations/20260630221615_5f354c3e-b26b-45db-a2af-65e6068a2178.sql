...
-- Admin function to fully delete an Import PO and reverse all related transactions.
-- Reverses: imp_payments + their journal entries, stock movements from carton posting, then deletes PO.
...
BEGIN
  ...
  -- 1. Reverse stock movements created from this PO's cartons (subtract back from stock)
  FOR v_mov IN
  ...
  -- 2. Collect & delete journal entries from imp_payments of this PO
  ...
  -- Also nuke any stray imp_payment journals referencing carton ids of this PO
     ...
     SELECT id FROM public.erp_journal_entries
      WHERE source_type IN ('imp_payment','imp_po','imp_carton')
        AND (source_id = _po_id
             ...
             OR source_id IN (SELECT id FROM public.imp_payments WHERE po_id = _po_id))
   );
  ...
  -- 3. Delete payments (restricted FK)
  DELETE FROM public.imp_payments WHERE po_id = _po_id;

  -- 4. Detach cargo bills (FK is ON DELETE SET NULL but be explicit)
  UPDATE public.imp_cargo_bills SET po_id = NULL WHERE po_id = _po_id;

  -- 5. Delete carton_items, cartons, po_items, status history, PO
  ...
  DELETE FROM public.imp_status_history WHERE po_id = _po_id;
