...
-- 1) Backup snapshot (reversible)
CREATE TABLE IF NOT EXISTS public._backup_status_migration_2026_07_12 AS
SELECT id, status::text AS old_status, payment_status AS old_payment_status,
       paid_at AS old_paid_at, updated_at AS old_updated_at, now() AS backed_up_at
FROM public.orders
WHERE status IN ('paid','paid_return');

-- 2) Data migration: paid (18) -> delivered
UPDATE public.orders
SET status = 'delivered', payment_status = 'paid'
WHERE status = 'paid';

-- 3) Data migration: paid_return (1) -> returned + payment_status paid (per instruction)
UPDATE public.orders
SET status = 'returned', payment_status = 'paid'
WHERE status = 'paid_return';

-- 4) Revenue filter: drop 'paid' (and 'paid_return' now empty) from revenue-recognition list
...
DECLARE
  ...
  _delivered_statuses order_status[] := ARRAY[
    'delivered','completed','partial_delivered','partial_return','exchange'
  ]::order_status[];
BEGIN
  IF NOT (
