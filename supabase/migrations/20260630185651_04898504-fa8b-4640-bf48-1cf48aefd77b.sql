...
-- Smarter FX fallback for Meta spend FIFO: prefer the ad account's own purchase rate
-- over a hardcoded 120, and only use erp_fx_rates / NULL as final fallback.
...
BEGIN
  ...
  IF remaining_to_consume > 0 THEN
    -- 1) Prefer this ad account's own avg effective rate from confirmed purchases
    ...
    -- 2) Then latest erp_fx_rates USD->BDT
    ...
    -- 3) Final safety net (only if nothing configured anywhere)
...
-- Reprice existing fx_fallback spend ledger rows that used the hardcoded 120
-- to the account's avg effective rate from confirmed purchases (if available).
WITH avg_rates AS (
  SELECT ad_account_id,
         CASE WHEN SUM(usd_amount) > 0 THEN SUM(total_bdt)/SUM(usd_amount) END AS r
    FROM public.meta_dollar_purchases
   WHERE status='confirmed'
   GROUP BY ad_account_id
)
UPDATE public.meta_ad_wallet_ledger l
   SET bdt_value = - ROUND(ABS(l.usd_delta) * a.r, 4)
  FROM avg_rates a
 WHERE l.ad_account_id = a.ad_account_id
   AND l.entry_type = 'spend'
   AND l.conversion_source = 'fx_fallback'
   AND a.r IS NOT NULL AND a.r > 0
   AND ABS((l.bdt_value / NULLIF(l.usd_delta,0)) - a.r) > 0.0001;

-- Also reprice meta_spend_consumptions.bdt_cost for the same rows
WITH avg_rates AS (
  SELECT ad_account_id,
         CASE WHEN SUM(usd_amount) > 0 THEN SUM(total_bdt)/SUM(usd_amount) END AS r
    FROM public.meta_dollar_purchases
...
UPDATE public.meta_spend_consumptions c
   SET bdt_cost = ROUND(c.usd_consumed * a.r, 4)
  FROM avg_rates a
 WHERE c.ad_account_id = a.ad_account_id
   AND c.conversion_source = 'fx_fallback'
   AND a.r IS NOT NULL AND a.r > 0;
