...
BEGIN
  ...
  ads AS (
    -- Read legacy ad/insight tables until Phase 3 (new Meta sync) is wired up
    SELECT l.product_id,
      SUM(COALESCE(ci.spend,0) * (l.allocation_percent / 100.0))::numeric AS meta_spend
    FROM public.erp_ad_product_links_legacy l
    JOIN public.marketing_campaign_insights_legacy ci ON ci.campaign_id = l.campaign_id
    WHERE l.brand_id = p_brand_id AND ci.date BETWEEN p_date_from AND p_date_to
  ...
  SELECT
    ...
    COALESCE(ads.meta_spend,0) + COALESCE(m.meta_ads_alloc,0) AS meta_ads,
    COALESCE(m.marketing_content,0) AS marketing_content,
    (COALESCE(a.revenue,0) - COALESCE(a.cogs,0)) AS gross_profit,
    (COALESCE(a.revenue,0) - COALESCE(a.cogs,0) - COALESCE(a.courier_cost,0)
      - COALESCE(r.return_loss,0) - COALESCE(e.exchange_loss,0)
      - COALESCE(ads.meta_spend,0) - COALESCE(m.meta_ads_alloc,0) - COALESCE(m.marketing_content,0)) AS net_profit,
    CASE WHEN COALESCE(a.delivered_qty,0) > 0
      THEN ((COALESCE(a.revenue,0) - COALESCE(a.cogs,0) - COALESCE(a.courier_cost,0)
          - COALESCE(r.return_loss,0) - COALESCE(e.exchange_loss,0)
          - COALESCE(ads.meta_spend,0) - COALESCE(m.meta_ads_alloc,0) - COALESCE(m.marketing_content,0)) / a.delivered_qty)
  ...
  LEFT JOIN ads        ON ads.product_id = p.id
  WHERE p.brand_id = p_brand_id
    AND (COALESCE(a.confirmed_qty,0) > 0 OR COALESCE(a.revenue,0) > 0 OR COALESCE(r.return_loss,0) > 0 OR COALESCE(e.exchange_loss,0) > 0 OR COALESCE(ads.meta_spend,0) > 0 OR COALESCE(m.meta_ads_alloc,0) > 0)
