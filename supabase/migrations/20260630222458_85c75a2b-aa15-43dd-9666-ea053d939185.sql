...
BEGIN
  ...
  -- 1. Reverse stock movements created from this PO's cartons
  FOR v_mov IN
  ...
  -- 2. Collect journal entry IDs referenced by this PO's payments
  SELECT COALESCE(array_agg(journal_entry_id), '{}')
    INTO v_je_ids
    FROM public.imp_payments
   WHERE po_id = _po_id AND journal_entry_id IS NOT NULL;

  -- 3. Detach cargo bills
  UPDATE public.imp_cargo_bills SET po_id = NULL WHERE po_id = _po_id;

  -- 4. Delete payments FIRST (this removes the FK reference to journal entries)
  DELETE FROM public.imp_payments WHERE po_id = _po_id;

  -- 5. Now safe to delete the journal entries that were referenced by those payments
  ...
  -- 6. Stray journal entries pointing at PO/cartons
  ...
  -- 7. Delete carton_items, cartons, po_items, status history, PO
