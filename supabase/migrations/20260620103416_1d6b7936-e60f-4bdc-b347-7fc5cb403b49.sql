...
DECLARE
  ...
  v_alloc_total numeric := 0; v_manual_total numeric := 0;
...
BEGIN
  ...
  -- Marketing: allocations + manual expenses combined
  WITH mkt AS (
    SELECT
      ...
      campaign_id,
      COALESCE(allocation_date, created_at::date) AS allocation_date,
      created_at
    ...
    SELECT
      'manual'::text AS kind,
      COALESCE(category::text, 'other') AS expense_type,
      amount::numeric AS amount,
      note,
      COALESCE(source::text, 'manual') AS source,
      campaign_id,
      COALESCE(date, created_at::date) AS allocation_date,
      created_at
    FROM public.mkt_manual_expenses
    WHERE product_id = p_product_id
      AND (p_brand_id IS NULL OR brand_id = p_brand_id)
      AND COALESCE(date, created_at::date) BETWEEN p_date_from AND p_date_to
  )
  SELECT
    COALESCE(SUM(amount),0),
    COALESCE(SUM(CASE WHEN expense_type IN ('meta_ads','meta_ads_manual','boost') OR (kind='manual' AND expense_type IN ('boost','meta','meta_ads','facebook_ads')) THEN amount ELSE 0 END),0),
    COALESCE(SUM(CASE WHEN kind='allocation' THEN amount ELSE 0 END),0),
    COALESCE(SUM(CASE WHEN kind='manual' THEN amount ELSE 0 END),0),
    COALESCE(jsonb_agg(jsonb_build_object('kind',kind,'expense_type',expense_type,'amount',amount,'note',note,'source',source,'campaign_id',campaign_id,'allocation_date',allocation_date,'created_at',created_at) ORDER BY allocation_date DESC),'[]'::jsonb)
  INTO v_marketing_total, v_meta_ads, v_alloc_total, v_manual_total, _marketing
  FROM mkt;
  ...
  _cost := jsonb_build_object('cogs',v_cogs,'courier_out',v_courier_out,'courier_return',v_courier_return,'packaging',v_packaging,'return_loss',v_return_loss,'exchange_loss',v_exchange_loss,'damage_loss',v_damage_loss,'refund_loss',v_refund_loss,'meta_ads',v_meta_ads,'marketing_content',GREATEST(v_marketing_total-v_meta_ads,0),'marketing_total',v_marketing_total,'marketing_allocations',v_alloc_total,'marketing_manual',v_manual_total);
